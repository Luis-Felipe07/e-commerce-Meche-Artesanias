...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    const buscadorInput = document.querySelector('.buscador input[name="q"]'); 
    ...
    const totalMetodosPagoSpan = document.getElementById('totalMetodosPago'); 
    const totalItemsCarritoDashSpan = document.getElementById('totalItemsCarritoDash'); 
    const contadorCarritoIcon = document.getElementById('contador-carrito-icon'); 
    ...
    const contenidoDetallePedido = document.getElementById('contenidoDetallePedido'); 
    ...
    const listaMetodosPagoDiv = document.getElementById('listaMetodosPago'); 
    ...
    const btnCerrarSesion1 = document.getElementById('cerrarSesion'); 
    const btnCerrarSesion2 = document.getElementById('cerrarSesionMenu'); 
    ...
    async function fetchAPI(url, options = {}) {
        ...
        try {
            ...
            if (response.status === 401 && !options.triedRefresh) {
                ...
                if (refreshed) {
                    console.log("[fetchAPI] Refresco de token exitoso. Reintentando la llamada original..."); 
                    config.headers['Authorization'] = `Bearer ${localStorage.getItem('accessToken')}`;
                    config.triedRefresh = true; 
                    response = await fetch(finalUrl, config); 
                    console.log(`[fetchAPI] Respuesta tras reintento para ${url}: Status ${response.status}`); 
                } else {
                    console.error("[fetchAPI] Falla en el refresco de token. Limpiando sesión y redirigiendo.");
            ...
            if (response.status !== 204 && contentType && contentType.includes("application/json")) { 
                try {
                    ...
                    console.log(`[fetchAPI] Datos JSON recibidos para ${url}:`, data); 
                } catch (e) {
                    console.error(`[fetchAPI] Error parseando JSON para ${url} (Status: ${response.status})`, e); 
                    if (!response.ok) {
                ...
                console.log(`[fetchAPI] Respuesta 204 No Content para ${url}`); 
                data = { success: true, status: 204 }; 
            } else {
                
                const textData = await response.text().catch(() => '');
                console.log(`[fetchAPI] Respuesta no JSON recibida para ${url} (Status: ${response.status}, Tipo: ${contentType}): ${textData.substring(0, 100)}`); 
            ...
            if (!response.ok) {
                ...
                console.error(`[fetchAPI] Error final en API ${response.status} para ${url}. Datos/Error:`, data); 
            ...
            console.log(`[fetchAPI] Llamada exitosa a ${url}`); 
            return data;

        } catch (error) {
            console.error(`[fetchAPI] Error CATCH en fetchAPI para ${finalUrl}:`, error.message); 
    ...
    async function refreshToken() {
        ...
        if (!refreshTokenValue) {
            console.warn("[refreshToken] No se encontró refresh token en localStorage."); 
            return false;
        }
        console.log(`[refreshToken] Usando refresh token: ${refreshTokenValue.substring(0, 15)}...`); // LOG REFRESH TOKEN VALUE 
        ...
        try {
            ...
            console.log("[refreshToken] Token refrescado y guardado exitosamente."); 
            return true;
        } catch (error) {
    ...
    async function verificarAutenticacionInicial() {
        ...
        if (!accessToken || isPotentiallyExpired) {
            console.log("[verificarAutenticacionInicial] Token ausente o potencialmente expirado. Intentando refrescar..."); 
            const refreshed = await refreshToken();
            if (!refreshed) {
                console.error("[verificarAutenticacionInicial] Refresco fallido. Redirigiendo a login."); 
                limpiarSesionYRedirigir();
                return false; 
            }
            // Si el refresco funcionó, el nuevo token ya está en localStorage
            console.log("[verificarAutenticacionInicial] Refresco inicial exitoso. Procediendo a verificar con API.");
        } else {
            console.log("[verificarAutenticacionInicial] Token local encontrado y parece válido. Verificando con API..."); 
        }

        // Ahora, verifica el token (original o recién refrescado) contra el backend
        try {
            // Uso la URL de  app 'usuarios' para verificar
    ...
    function actualizarUIUsuario(usuario) {
        ...
        if (formEditarPerfil) {
            ...
            if (formEditarPerfil.telefonoPerfil) formEditarPerfil.telefonoPerfil.value = usuario.telefono || ''; 
        ...
        menuSecciones.forEach(enlace => {
            enlace.addEventListener('click', function (e) {
                ...
                if (seccionAttr && seccionAttr !== '#') {
                    ...
                    if (parentDropdown) parentDropdown.classList.remove('activo'); 
                }
            });
        });
